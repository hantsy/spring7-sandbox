...
Spring 7 and Spring Boot 4 are aligned with **Jakarta EE 11**, which includes Jakarta Persistence 3.2. A notable improvement in Spring 7 is the enhanced JPA support, which allows direct injection of `EntityManager` and `EntityManagerFactory` into Spring beans via standard `@Autowired` or constructor injection, eliminating the strict requirement for `@PersistenceContext` or `@PersistenceUnit`. Additionally, Spring 7 embraces Servlet 6.1 totally, and Spring Boot is aligned with the latest Servlet containers, such as Apache Tomcat, Eclipse Jetty, etc. Unfortunately, Undertow has not released a Servlet 6.1 comptable version when Spring Boot v4 is release.
