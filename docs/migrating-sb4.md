...
Spring 7 and Spring Boot 4 are aligned with **Jakarta EE 11**, which includes Jakarta Persistence 3.2. A notable improvement in Spring 7 is the enhanced JPA support, which allows direct injection of `EntityManager` and `EntityManagerFactory` into Spring beans via standard `@Autowired` or constructor injection, eliminating the strict requirement for `@PersistenceContext` or `@PersistenceUnit`.
